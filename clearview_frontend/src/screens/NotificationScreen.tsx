...
export default function NotificationsView() {
  ...
  const allItems = useMemo(() => {
    const items: {
      ...
      ...budgetAlerts.map((a) => ({
        ...a,
        id: `alert-${a.title}-${a.date.getTime()}`,
      })),
      ...notifications.map((n) => ({
  ...
  const renderIcon = (type: string) => {
    if (type === "alert") {
      return (
        <div
          ...
          style={{ width: "40px", height: "40px", backgroundColor: "#FEF2F2" }}
        >
          <svg
    ...
    if (type === "envelope") {
      return (
        <div
          className="rounded-3 d-flex align-items-center justify-content-center flex-shrink-0"
          style={{ width: "40px", height: "40px", backgroundColor: "#EDF2FF" }}
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
    ...
    >
      ...
      <div className="d-flex justify-content-between align-items-center mb-4 pt-2">
        <p className="mb-0 text-secondary" style={{ fontSize: "15px" }}>
          You have{" "}
          <strong className="text-dark fw-bold">{unreadCount} unread</strong>{" "}
